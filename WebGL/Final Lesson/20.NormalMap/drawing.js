...
var vertexNormalHandle,vertexPositionHandle,vertexUVHandle ,textureFileHandle , textureInfluenceHandle, normalInfluenceHandle, matrixPositionHandle, materialDiffColorHandle, lightDirectionHandle, lightPositionHandle, lightColorHandle, lightTypeHandle, eyePositionHandle, materialSpecColorHandle, materialSpecPowerHandle;
...
function updateNormalInfluence(val){
	normalInfluence = val;
}
...
function loadShaders(){
		...
		utils.loadFiles([shaderDir + 'vs_map.glsl',
						 shaderDir + 'fs_map.glsl'
						 ], 
						 function(shaderText){
		...
		vertexUV2Handle = gl.getAttribLocation(shaderProgram, 'inUV2s');
		...
		lightMapFileHandle = gl.getUniformLocation(shaderProgram, 'lightMapFile');
		textureInfluenceHandle = gl.getUniformLocation(shaderProgram, 'textureInfluence');
		normalInfluenceHandle = gl.getUniformLocation(shaderProgram, 'normalInfluence');
		...
		utils.get_json(modelsDir + modelName, function(loadedModel){
			...
			sceneObjects = loadedModel.meshes.length; 
				...
				for (i=0; i < sceneObjects ; i++) { 
					...
					for (n = 0; n < loadedModel.meshes[i].vertices.length/3; n++){
						...
						if(UVFileNamePropertyIndex>=0){
							objVertex.push( loadedModel.meshes[i].texturecoords[0][n*2],
											1.0 - loadedModel.meshes[i].texturecoords[0][n*2+1]);
						} else {
					...
					//Correcting the orientation of the suite
...
function drawScene(){
		...
		for(i=0; i < sceneObjects; i++){
			...
			gl.uniform1f(normalInfluenceHandle, normalInfluence);	
