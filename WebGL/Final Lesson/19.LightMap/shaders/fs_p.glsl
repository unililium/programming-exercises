...
void main() { 
	...
	if(lightType == 5){
		gl_FragColor = lightMapTextureColorMixture * diffuseTextureColorMixture;
	}else{
		//Computing the diffuse component of light (no ambient considered)
		vec4 diffuse = diffuseTextureColorMixture * lightMapTextureColorMixture * lightColor * clamp(dot(nlightDirection, nNormal), 0.0, 1.0) * lightDimension;	
