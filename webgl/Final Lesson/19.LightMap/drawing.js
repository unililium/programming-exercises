...
var vertexUV2Handle = new Array(2);
var textureFileHandle = new Array(2);
var lightMapFileHandle = new Array(2);
var textureInfluenceHandle = new Array(2);
var lightMapInfluenceHandle = new Array(2);
...
var lightMapTextureObj;	//Texture material
...
function updateShader(val){
	...
	if(currentShader==1) {
		document.getElementById("nol").disabled=true;
		document.getElementById("yesl").checked=true;
		currentLightType = 1;
	}
	else document.getElementById("nol").disabled=false;
}

function updateLightMapInfluence(val){
	lightMapInfluence = val;
}
...
function loadShaders(){
		...
		for(i = 0; i <2; i++){
			...
			vertexUV2Handle[i] = gl.getAttribLocation(shaderProgram[i], 'inUV2s');
			...
			lightMapFileHandle[i] = gl.getUniformLocation(shaderProgram[i], 'lightMapFile');
			textureInfluenceHandle[i] = gl.getUniformLocation(shaderProgram[i], 'textureInfluence');
			lightMapInfluenceHandle[i] = gl.getUniformLocation(shaderProgram[i], 'lightMapInfluence');
...
function loadModel(modelName){
		...
		//lightMapTextureObj = utils.getTexture(modelsDir+"LightMap.png");

		utils.get_json(modelsDir + modelName, function(loadedModel){
				
			sceneObjects = loadedModel.meshes.length - 1; // ***HACK for Hogwart file (-1 is because the last obj
				...
				for (i=0; i < sceneObjects ; i++) { 
					...
					if (loadedModel.meshes[i].texturecoords){
						...
						console.log("UV2ss: " + loadedModel.meshes[i].texturecoords[1].length);
					} else {
					...
					for (n = 0; n < loadedModel.meshes[i].vertices.length/3; n++){
						...
						if(UVFileNamePropertyIndex>=0){
							...
							objVertex.push( loadedModel.meshes[i].texturecoords[1][n*2],
											loadedModel.meshes[i].texturecoords[1][n*2+1]);											

						} else {
							objVertex.push( 0.0, 0.0, 0.0, 0.0);
						}
					}
					...
					if(UVFileNamePropertyIndex>=0){
						...
						var getTexture = function(image_URL){
							...
							//HACK for this model. Object 8 has the LM file stored as its texture,
							//so we can load it here.
							if(i==8) lightMapTextureObj	= getTexture(modelsDir + imageName);
...
function drawScene(){
		...
		for(i=0; i < sceneObjects; i++){
			...
			gl.uniform1f(lightMapInfluenceHandle[currentShader], lightMapInfluence);
			...
			gl.uniform1i(lightMapFileHandle[currentShader], 1);		//Texture channel 1 used for LM
			if (nTexture[i]==true && lightMapTextureObj.webglTexture) {
				gl.activeTexture(gl.TEXTURE1);
				gl.bindTexture(gl.TEXTURE_2D, lightMapTextureObj.webglTexture);	
			} 
			...
			gl.vertexAttribPointer(vertexPositionHandle[currentShader], 3, gl.FLOAT, gl.FALSE, 4 * 10, 0);
			
			gl.enableVertexAttribArray(vertexNormalHandle[currentShader]);
			gl.vertexAttribPointer(vertexNormalHandle[currentShader], 3, gl.FLOAT, gl.FALSE, 4 * 10, 4 * 3);	

			gl.vertexAttribPointer(vertexUVHandle[currentShader], 2, gl.FLOAT, gl.FALSE, 4*10, 4*6);                   
			gl.enableVertexAttribArray(vertexUVHandle[currentShader]);		

			gl.vertexAttribPointer(vertexUV2Handle[currentShader], 2, gl.FLOAT, gl.FALSE, 4*10, 4*8);
			gl.enableVertexAttribArray(vertexUV2Handle[currentShader]);				
